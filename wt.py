...
def lg():
  ...
  print(f'{NC}•    {C}01{NC}) {G}Bing!{NC}       {C}05{NC}) {G}MOZ Checker{NC}')
  print(f'{NC}•    {C}02{NC}) {G}Google{NC}      {C}06{NC}) {G}Chat GPT 3.5{NC}')
  print(f'{NC}•    {DY}03{DW}) {DR}Yandex{NC}      {C}07{NC}) {G}Grab URL List{NC}')
  print(f'{NC}•    {C}04{NC}) {G}Check Index{NC} {C}08{NC}) {G}Tutor & Donation{NC}')
...
try:
  import re, json, random, openai, warnings, httplib2, queue, platform, traceback, threading, urllib.parse, requests.packages.urllib3
  ...
  os.system('python3 -m pip install fake_useragent oauth2client requests bs4 openai python-dotenv')
  lg()
  print(f'{NC} • • • • • • • • • • • • • {BR}  WEBMASTER  {NC}')
  ...
  try:
    ...
    def cekMOZ(site):
      dat = {
        'domain': site, 'rank': 'DA, PA, SS, BL',
        #'spam_score': 'SPAM SCORE', 'ref_domain': 'SUMBER BACKLINK',
        #'anchor_text': 'KEYWORD BACKLINK'
      }
      res = bs(ses.post(f'https://xeo.my.id/venus/checker/moz.cgi', headers=hider, data=dat, cookies={'CONSENT': 'YES+1'}, timeout=10, verify=False).text, 'html.parser').findAll('table', {'style': 'margin: 0 0 4px 0;'})
      a = res[0]
      if a:
        lg()
        print(f'{NC} • • • • • • • • • • • • • • • • • • • •')
        print(f'{NC} • • • • • • • • • • • • • {BR}     MOZ     {NC}')
        aa = a.findAll('td')
        print(f'{NC}• {G}✓ {NC}=> {C}{site}{NC}')
        print(f'{NC}• [ {Y}DA {NC}] => {G}{aa[6].text}    {NC}[ {Y}BL {NC}] => {G}{aa[9].text}{NC}')
        print(f'{NC}• [ {Y}PA {NC}] => {G}{aa[7].text}    {NC}[ {Y}MT {NC}] => {G}{aa[10].text}{NC}')
        print(f'{NC}• [ {Y}SS {NC}] => {G}{aa[8].text}    {NC}[ {Y}MR {NC}] => {G}{aa[11].text}{NC}')
        print(f'{NC} • • • • • • • • • • • • • {BW}   CHECKER   {NC}')
        print(f'{NC} • • • • • • • • • • • • • • • • • • • •')
        exit()
      else:
        print(f'{NC}   • {DW}=> {DR}Wait {Y}30s{DW}..!!{NC}')
        sleep(3)
        main()
    ...
    def grabL(site):
      count = 0
      url = site + '/feeds/posts/default'
      res = bs(ses.get(url, headers=hider, cookies={'CONSENT': 'YES+1'}, timeout=10, verify=False).text, 'html.parser').findAll('link', {'rel': 'alternate'})
      if res:
        for url in res:
          count += 1
          urls = url['href']
          print(f'{NC}  • {C}{count}{NC}) {G}Grabbed{DW}!! => ...{NC}{C}{urls[12:35]}{DW}...{NC}', sep=' ', end='\n', file=sys.stdout, flush=False)
          dapat.append(f'{urls}')
          with open('list.txt', 'a') as (s):
            s.write(f'{urls}\n')
          s.close()
        else:
          print(f'{NC} • • • • • • • • • • • • • • • • • • • •')
          print(f'{NC} • • • • • • • • • • • • • {BR}    GRAB!    {NC}')
          print(f'{NC}•  ✓{G} DONE{NC}..!!')
          print(f'{NC}• {G}Grabbed {C}{len(dapat)} {Y}URL {NC}')
          print(f'{NC}• {R}Remaining Undetect{NC}..!!')
          print(f'{NC}• {C}Saved {DW}=> {NC}{G}list.txt{NC}')
          print(f'{NC} • • • • • • • • • • • • • {BW}     URL     {NC}')
          print(f'{NC} • • • • • • • • • • • • • • • • • • • •')
          exit()
      else:
        exit(f' • EXIT')
    ...
    def main():
      ...
      #MOZ Checker
        ...
        lg()
        print(f'{NC} • • • • • • • • • • • • • {BW} MOZ CHECKER {NC}')
        site = input(f'  • {Y}Input {G}URL {C}Site {NC}=>{DW} ')
        sipot = site.split('http')[0]
        if site != sipot:
          lg()
          print(f'{NC} • • • • • • • • • • • • • {BW}     MOZ     {NC}')
          print(f'{NC}  • • • • • Start {C}MOZ CHECKER! {NC}• • • • •')
          tungs(3)
          cekMOZ(site)
        else:
          print(f'{NC}   • {DW}=> {DR}Wrong Input{DW}..!!{NC}')
      ...
      #Chat GPT
        ...
        lg()
        print(f'{NC} • • • • • • • • • • • • • {BW}  CHAT GPT3  {NC}')
        if os.path.isfile('GPT/.env'):
          while True:
            myQ = input(f'{NC}  • {C}Me {NC}=>{DW} ')
            askGPT(myQ)
        else:
          print(f'{NC}  • {DG}API Keys {DY}Not Found{DW} => {DC}GPT/.env{NC}')
          sleep(3)
          exit()
      ...
      #Grab URL List
        ...
        lg()
        print(f'{NC} • • • • • • • • • • • • • {BW}  GRAB LIST  {NC}')
        site = input(f'  • {Y}Input {G}URL {C}Site {NC}=>{DW} ')
        sipot = site.split('http')[0]
        if site != sipot:
          lg()
          print(f'{NC} • • • • • • • • • • • • • {BW}  GRAB LIST  {NC}')
          print(f'{NC}  • • • •  Start {C}GRAB {Y}URL LIST{NC}!  • • • •')
          tungs(3)
          grabL(site)
        else:
          print(f'{NC}   • {DW}=> {DR}Wrong Input{DW}..!!{NC}')
        ...
        lg()
        print(f'{NC} • • • • • • • • • • • • • {BR}  WEBMASTER  {NC}')
        print(f'{NC} • {Y}Opening {R}Browser{NC}..!!')
        print(f'{NC} • {Y}Redirect To {C}Page{NC}..!!')
        print(f'{NC} • • • • • • • • • • • • • {BW}  TUTORIAL!  {NC}')
        tungs(3)
        os.system(f'xdg-open https://trakteer.id/ggtutor/showcase/webmaster-indexing-tools-tutorial-t3tcu')
        exit(f'                    \r')
